...
begin
  ...
  alter table public.market_source_catalog
    add column if not exists catalog_status text not null default 'shadow',
    add column if not exists catalog_block_reason text,
    add column if not exists catalog_quality_score numeric(10, 2) not null default 0,
  ...
  with classified as (
    select
      m.id,
      greatest(m.snapshot_captured_at, m.quote_fetched_at) as next_last_market_signal_at,
      case
        ...
        when coalesce(m.market_coverage_count, 0) < 2
          then 'shadow'
        when greatest(m.snapshot_captured_at, m.quote_fetched_at) is null
          then 'shadow'
        when greatest(m.snapshot_captured_at, m.quote_fetched_at) < (now() - interval '4 hours')
          then 'shadow'
        else 'scannable'
      ...
      case
        ...
        when greatest(m.snapshot_captured_at, m.quote_fetched_at) is not null
          and greatest(m.snapshot_captured_at, m.quote_fetched_at) < (now() - interval '4 hours')
      ...
      (
        ...
        case
          when greatest(m.snapshot_captured_at, m.quote_fetched_at) >= (now() - interval '2 hours') then 8
          when greatest(m.snapshot_captured_at, m.quote_fetched_at) >= (now() - interval '6 hours') then 4
          when greatest(m.snapshot_captured_at, m.quote_fetched_at) is not null then -6
          else -10
        end
      ) as quality_base
